...
public class FolderAdapter extends BaseAdapter {
        ...
        if (holder != null) {
            try {
                if (i == 0) {
                    ...
                    holder.size.setText(String.format("%d%s", getTotalImageSize(), mContext.getResources()
    ...
    class ViewHolder {
        ...
        void bindData(Folder data) {
            ...
            // 显示图片
            if (data.cover != null) {
                Picasso.with(mContext).load(new File(data.cover.path)).placeholder(R.drawable.default_error)
                        .resizeDimen(R.dimen.folder_cover_size, R.dimen.folder_cover_size).centerCrop().into(
                        cover);
            } else {
