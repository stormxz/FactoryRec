...
import com.example.factoryrec.R.styleable;
 ...
 * <p>
 * v prefixes - coordinates, translations and distances measured in screen (view) pixels s prefixes - coordinates,
 * translations and distances measured in source image pixels (scaled)
 */
public class ScaleImageView extends ImageView {
    ...
    public ScaleImageView(Context context, AttributeSet attr) {
        ...
        if (attr != null) {
            TypedArray typedAttr = getContext().obtainStyledAttributes(attr, styleable.ScaleImageView);
            if (typedAttr.hasValue(styleable.ScaleImageView_assetName)) {
                String assetName = typedAttr.getString(styleable.ScaleImageView_assetName);
                if (assetName != null && assetName.length() > 0) {
                    setImage(ImageSource.asset(assetName).tilingEnabled());
                }
            }
            if (typedAttr.hasValue(styleable.ScaleImageView_src)) {
                int resId = typedAttr.getResourceId(styleable.ScaleImageView_src, 0);
                if (resId > 0) {
                    setImage(ImageSource.resource(resId).tilingEnabled());
                }
            }
            if (typedAttr.hasValue(styleable.ScaleImageView_panEnabled)) {
                setPanEnabled(typedAttr.getBoolean(styleable.ScaleImageView_panEnabled, true));
            }
            if (typedAttr.hasValue(styleable.ScaleImageView_zoomEnabled)) {
                setZoomEnabled(typedAttr.getBoolean(styleable.ScaleImageView_zoomEnabled, true));
            }
            if (typedAttr.hasValue(styleable.ScaleImageView_quickScaleEnabled)) {
                setQuickScaleEnabled(typedAttr.getBoolean(styleable.ScaleImageView_quickScaleEnabled, true));
            }
            if (typedAttr.hasValue(styleable.ScaleImageView_tileBackgroundColor)) {
                setTileBackgroundColor(typedAttr.getColor(styleable.ScaleImageView_tileBackgroundColor,
                                                          Color.argb(0, 0, 0, 0)));
            }
            typedAttr.recycle();
        }
     ...
     * setting, scale and center. This is the best method to use when you want scale and center to be restored after
     * screen orientation change; it avoids any redundant loading of tiles in the wrong orientation.
     *
     * @param imageSource
     *         Image source.
     * @param state
    ...
    /**
     * Set the image source from a bitmap, resource, asset, file or other URI, providing a preview image to be displayed
     * until the full size image is loaded.
     * <p>
     * You must declare the dimensions of the full size image by calling {@link ImageSource#dimensions(int, int)} on the
     * imageSource object. The preview source will be ignored if you don't provide dimensions, and if you provide a
     * bitmap for the full size image.
     *
     * @param imageSource
     *         Image source. Dimensions must be declared.
     * @param previewSource
     *         Optional source for a preview image to be displayed and allow interaction while the full
     *         size image loads.
     */
    ...
    /**
     * Set the image source from a bitmap, resource, asset, file or other URI, providing a preview image to be displayed
     * until the full size image is loaded, starting with a given orientation setting, scale and center. This is the
        ...
        this.detector = new GestureDetector(context, new GestureDetector.SimpleOnGestureListener() {
            ...
            public boolean onFling(MotionEvent e1, MotionEvent e2, float velocityX, float velocityY) {
                ...
                if (panEnabled && readySent && vTranslate != null && e1 != null && e2 != null && (Math.abs(
                        e1.getX() - e2.getX()) > 50 || Math.abs(e1.getY() - e2.getY()) > 50) && (Math.abs(
                        velocityX) > 500 || Math.abs(velocityY) > 500) && !isZooming) {
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (maxTouchCount > 0) {
                    if (touchCount >= 2) {
                        ...
                        if (zoomEnabled && (distance(vCenterStart.x, vCenterEndX, vCenterStart.y, vCenterEndY)
                            ...
                            if (scale <= minScale()) {
                                ...
                                // Translate to place the source image coordinate that was at the center of the pinch at
                                // the start
    ...
    private void doubleTapZoom(PointF sCenter, PointF vFocus) {
        ...
        if (doubleTapZoomStyle == ZOOM_FOCUS_CENTER_IMMEDIATE) {
            ...
            new AnimationBuilder(targetScale, sCenter, vFocus).withInterruptible(false).withDuration(
                    doubleTapZoomDuration).start();
        }
        invalidate();
    }
     ...
     * calculate the scaling and tiling required. Once the view is setup, tiles are displayed as they are loaded.
     */
    @Override
    protected void onDraw(Canvas canvas) {
        ...
        if (tileMap != null && isBaseLayerReady()) {
            ...
            if (debug) {
                ...
                canvas.drawText(
                        "Translate: " + String.format(Locale.ENGLISH, "%.2f", vTranslate.x) + ":" + String
                                .format(Locale.ENGLISH, "%.2f", vTranslate.y), 5, 35, debugPaint);
                PointF center = getCenter();
                canvas.drawText(
                        "Source center: " + String.format(Locale.ENGLISH, "%.2f", center.x) + ":" + String
                                .format(Locale.ENGLISH, "%.2f", center.y), 5, 55, debugPaint);
     ...
     */
    private void setMatrixArray(float[] array, float f0, float f1, float f2, float f3, float f4, float f5,
            float f6, float f7) {
    ...
    /**
     * Check whether view and image dimensions are known and either a preview, full size image or base layer tiles are
     * loaded. First time, send ready event to listener. The next draw will display an image.
     */
    ...
    /**
     * Check whether either the full size bitmap or base layer tiles are loaded. First time, send image loaded event to
     * listener.
     */
     ...
     * screen. Minimum scale is set so one dimension fills the view and the image is centered on the other dimension.
     * Used to calculate what the target of an animation should be.
     *
     * @param center
     *         Whether the image should be centered in the dimension it's too small to fill. While animating this
    ...
    private void fitToBounds(boolean center, ScaleAndTranslate sat) {
        ...
        float yPaddingRatio =
                getPaddingTop() > 0 || getPaddingBottom() > 0 ? getPaddingTop() / (float) (getPaddingTop()
     ...
     * Minimum scale is set so one dimension fills the view and the image is centered on the other dimension.
     *
     * @param center
     *         Whether the image should be centered in the dimension it's too small to fill. While animating this
     *         can be false to avoid changes in direction as bounds are reached.
    ...
    private static class TilesInitTask extends AsyncTask<Void, Void, int[]> {
        ...
        public TilesInitTask(ScaleImageView view, Context context,
            ...
            this.decoderFactoryRef = new WeakReference<DecoderFactory<? extends ImageRegionDecoder>>(
                    decoderFactory);
            this.source = source;
        }
    ...
    private class BitmapLoadTask extends AsyncTask<Void, Void, Integer> {
        ...
        public BitmapLoadTask(ScaleImageView view, Context context,
    ...
    private synchronized void onImageLoaded(Bitmap bitmap, int sOrientation, boolean bitmapIsCached) {
        ...
        // If actual dimensions don't match the declared size, reset everything.
        if (this.sWidth > 0 && this.sHeight > 0 && (this.sWidth != bitmap.getWidth() || this.sHeight != bitmap
                .getHeight())) {
            reset(false);
        }
        if (this.bitmap != null) {
    ...
    /**
     * Helper method for load tasks. Examines the EXIF info on the image file to determine the orientation. This will
     * only work for external files, not assets, resources or other URIs.
     */
    ...
    private void execute(AsyncTask<Void, Void, ?> asyncTask) {
        ...
        if (parallelLoadingEnabled && VERSION.SDK_INT >= 11) {
            try {
                ...
                Method executeMethod = AsyncTask.class.getMethod("executeOnExecutor", Executor.class,
                                                                 Object[].class);
     ...
     * already, to the rectangle of the image that needs to be loaded.
     */
     ...
     * After you have called this method, the view can be re-used by setting a new image. Settings are remembered but
     * state (scale and center) is forgotten. You can restore these yourself if required.
     */
    public void recycle() {
     ...
     * file or asset, and you cannot use a custom decoder when using layout XML to set an asset name. Your class must
    ...
    /**
     * Swap the default region decoder implementation for one of your own. You must do this before setting the image
     * file or asset, and you cannot use a custom decoder when using layout XML to set an asset name.
     *
     ...
     *         The {@link DecoderFactory} implementation that produces {@link ImageDecoder}
     *         instances.
     */
     ...
     * allowed scale in terms of the minimum pixel density. This avoids the problem of 1:1 scale still being too small
     * on a high density screen. A sensible starting point is 160 - the default used by this view.
     *
     * @param dpi
     ...
     * quality image. 160-240dpi will usually be enough. This should be called before setting the image source, because
     * it affects which tiles get loaded. When using an untiled source image this method has no effect.
     *
    ...
    /**
     * Call to find whether the view is initialised, has dimensions, and will display an image on the next draw. If a
     * preview has been provided, it may be the preview that will be displayed and the full size image may still be
     * loading. If no preview was provided, this is called once the base layer tiles of the full size image are loaded.
     */
    ...
    /**
     * Called once when the view is initialised, has dimensions, and will display an image on the next draw. This is
     * triggered at the same time as {@link OnImageEventListener#onReady()} but allows a subclass to receive this event
    ...
    /**
     * Call to find whether the main image (base layer tiles where relevant) have been loaded. Before this event the
     * view is blank unless a preview was provided.
     */
     ...
     * {@link #getSHeight()} for the apparent width.
     */
     ...
     * {@link #getSWidth()} for the apparent height.
     */
     ...
     * interpreted as a zoom out gesture - if the scale is greater than 90% of this value, a double tap zooms out. Avoid
     ...
     * will be ignored. A sensible starting point is 160 - the default used by this view.
     *
     * @param dpi
     *         New value for double tap gesture zoom scale.
     */
    ...
    /**
     * Toggle parallel loading. When enabled, tiles are loaded using the thread pool executor available in SDK 11+. In
     * older versions this has no effect. Parallel loading may use more memory and there is a possibility that it will
     * make the tile loading unreliable, but it reduces the chances of an app's background processes blocking loading.
     *
     ...
     *         Target center point
     *
     * @return {@link AnimationBuilder} instance. Call {@link AnimationBuilder#start()} to start the
     * anim.
     */
    ...
    /**
     * An event listener for animations, allows events to be triggered when an animation completes, is aborted by
     * another animation starting, or is aborted by a touch event. Note that none of these events are triggered if the
     * activity is paused, the image is swapped, or in other cases where the view's internal state gets wiped or draw
     * events stop.
     */
    ...
    public interface OnImageEventListener {
        ...
        /**
         * Called when the dimensions of the image and view are known, and either a preview image, the full size image,
         * or base layer tiles are loaded. This indicates the scale and translate are known and the next draw will
         * display an image. This event can be used to hide a loading graphic, or inform a subclass that it is safe to
         * draw overlays.
         */
        void onReady();

        /**
         * Called when the full size image is ready. When using tiling, this means the lowest resolution base layer of
         * tiles are loaded, and when tiling is disabled, the image bitmap is loaded. This event could be used as a
         * trigger to enable gestures if you wanted interaction disabled while only a preview is displayed, otherwise
         * for most cases {@link #onReady()} is the best event to listen to.
         */
        void onImageLoaded();

        /**
         * Called when a preview image could not be loaded. This method cannot be relied upon; certain encoding types of
         * supported image formats can result in corrupt or blank images being loaded and displayed with no detectable
        ...
        /**
         * Indicates an error initiliasing the decoder when using a tiling, or when loading the full size bitmap when
         * tiling is disabled. This method cannot be relied upon; certain encoding types of supported image formats can
         * result in corrupt or blank images being loaded and displayed with no detectable error.
         *
         * @param e
        ...
        /**
         * Called when an image tile could not be loaded. This method cannot be relied upon; certain encoding types of
         * supported image formats can result in corrupt or blank images being loaded and displayed with no detectable
         * error. Most cases where an unsupported file is used will result in an error caught by
         * {@link #onImageLoadError(Exception)}.
         *
         * @param e
         *         The exception thrown. This error is logged by the view.
         */
